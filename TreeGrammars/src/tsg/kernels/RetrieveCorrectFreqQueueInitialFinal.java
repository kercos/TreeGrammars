...
public class RetrieveCorrectFreqQueueInitialFinal extends RetrieveCorrectFreq {

	public static int numberOfFragmentsPerThreads = 5;
	public static int printProgressEvery = 10;	
	...
	public RetrieveCorrectFreqQueueInitialFinal(ArrayList<TSNodeLabel> treebank, File fragmentsFile,
	...
	public void retriveCorrectFreqResume() {
		...
		if (partialFragments) {
			...
			long[] count = TSNodeLabel.countFragmentInTreebankInitialFinal(treebank, fragment);
			countString = count[0] + "\t" + count[1] + "\t" + count[2];
		}
		try {
			fragmentWriter.write(fragment.toString(false, true) + "\t" + countString + "\n");
		} catch (IOException e) {
			e.printStackTrace();
			return;
		}		
	...
	private void processInputOutputSingleThreadFragments() throws Exception {
		while(fragmentScanner.hasNextLine()) {
			...
			long[] count = TSNodeLabel.countFragmentInTreebankInitialFinal(treebank, fragment);	
			fragmentWriter.write(fragment.toString(false, true) + "\t" + 
	...
	private synchronized void addFragmentsToQueue(TSNodeLabel[] currentFragments, 
		...
		if (firstFragmentIndex==fragmentWrittenCounter) {
			...
			for(; i<numberOfFragmentsPerThreads; i++) {
				...
				long[] freqInitialFreq =  fragmentsCounts[i];
				String freqString = freqInitialFreq[0] + "\t" + freqInitialFreq[1]+ "\t" + freqInitialFreq[2];
				String newFragmentCount = currentFragment.toString(false, true) + "\t" + freqString + "\n";
	...
	private class CountFragmentsThread extends Thread {
		...
		protected long[] getCount(TSNodeLabel currentFragment) {
			return TSNodeLabel.countFragmentInTreebankInitialFinal(treebank, currentFragment);
		}
	...
	private class CountPartialFragmentsThread extends CountFragmentsThread {
		...
		protected long[] getCount(TSNodeLabel currentFragment) {
			return TSNodeLabel.countPartialFragmentInTreebankInitial(treebank, currentFragment);
		}
	}	
	...
	public static void main(String[] args) throws Exception {
		...
		String usage = "USAGE: java RetrieveCorrectFreqQueueInitial [-partialFragments:false] [-threads:1] " +
		...
		ArrayList<TSNodeLabel> treebank = Wsj.getTreebank(treebankFile);		
		RetrieveCorrectFreqQueueInitialFinal RCF = new RetrieveCorrectFreqQueueInitialFinal(treebank, inputFile, outputFile, partialFragments, threads);
		RCF.run();
