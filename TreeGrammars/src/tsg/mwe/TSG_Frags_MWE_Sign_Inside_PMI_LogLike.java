...
public class TSG_Frags_MWE_Sign_Inside_PMI_LogLike {
	...
	File fragFreqFile, fragInsideRatioFile, outputFileStats;
	String FileStatsName, FileFragPrefix;
	String outputDir;	
	int totalFrags, acquiredFrags;
	HashSet<TSNodeLabel> frags = new HashSet<TSNodeLabel>(); 
	...
	HashMap<TSNodeLabel, double[]> fragLogInsideRatio = new HashMap<TSNodeLabel, double[]>(); 
	
	public TSG_Frags_MWE_Sign_Inside_PMI_LogLike(File fragFreqFile, File fragInsideRatioFile, String outputDir) throws Exception {
		FileStatsName = "stats.txt";
		FileFragPrefix = "frags_"; // lexCount_signature
		this.fragFreqFile = fragFreqFile;
		this.fragInsideRatioFile = fragInsideRatioFile;
		this.outputDir = new File(outputDir) + "/";
		this.outputFileStats = new File(this.outputDir + FileStatsName);		
		readFragmentBank();
		readFragmentInsideRatioFile();
		printFileStats();
		printFragsMPI();		
	}
	...
	private void readFragmentBank()  throws Exception {
		...
		Scanner scan = new Scanner(fragFreqFile);
		int index = 0;
	...
	private void readFragmentInsideRatioFile()  throws Exception {
		PrintProgress progress = new PrintProgress("Reading fragments", 100, 0);
		Scanner scan = new Scanner(fragInsideRatioFile);
		int localFrags=0;
		while(scan.hasNextLine()) {
			localFrags++;
			String line = scan.nextLine();
			...
			if (!frags.contains(frag))
				continue;			
			double insideLogRatio = Double.parseDouble(lineSplit[1]);
			fragLogInsideRatio.put(frag, new double[]{insideLogRatio});
		}
		progress.end();
		System.out.println("Total frags: " + localFrags);
	}
	...
	private void printFileStats() throws Exception {		
		...
		pw.println("Input file: " + fragFreqFile);		
		...
		for(Entry<Integer, HashMap<String, HashMap<TSNodeLabel, int[]>>> a : 
			...
			for(Entry<String, HashMap<TSNodeLabel, int[]>> b : a.getValue().entrySet()) {
				...
				for(Entry<TSNodeLabel, int[]> f : table.entrySet()) {
					...
					double insideLogRatio = fragLogInsideRatio.get(frag)[0];
					...
					pw.println(fronteers + "\t" + frag.toStringStandard() + "\t" + freq + "\t" + 
							...
							relFreqEntropy[0] + "\t" + relFreqEntropy[1] + "\t" + insideLogRatio);
				}
				pw.close();				
			}			
		}
	...
	public static void main(String[] args) throws Exception {
		...
		String usage = "java TSG_Frags_MWE_Sign_Inside_PMI_LogLike fragFreqFile fragInsideRatioFile outputDir addRootLabelsInSignature encodePOSLabelsInSignature fragFreqThreshold SignFragThreshold";
		if (args.length!=7) {
			System.err.println("Wrong number of parameters");
			System.err.println(usage);
			return;
		}
		if (!new File(args[2]).exists() || !new File(args[2]).isDirectory()) {
			System.err.println("OutputDir does not exists or is not a directory");
			System.err.println(usage);
			return;
		}
		addRootLabelsInSignature = Boolean.parseBoolean(args[3]);
		encodePOSLabelsInSignature = Boolean.parseBoolean(args[4]);
		FragFreqThreshold = Integer.parseInt(args[5]);
		SignFragCountThreshold = Integer.parseInt(args[6]);
		new TSG_Frags_MWE_Sign_Inside_PMI_LogLike(new File(args[0]), new File(args[1]), args[2]);
