...
public class Sample_NYT {

	
	@SuppressWarnings("resource")
	public static void main(String[] args) throws Exception {
		
		String usage = "java Sample_NYT inputDir outputFile skip";
		...
		System.out.println("Skipping lines: " + skip);
		
		int skipCount = 0;
		String line;
		PrintProgress progress = new PrintProgress("Reading lines in files", 100, 0);
		...
		for(File f : files) {
			...
			while(scan.hasNextLine()) {				
				...
				if (++skipCount==skip) {
					if (!checkLine(line)) {
						System.err.println("Error in file " + f.getName() + " at line " + lineNumber);
			...
			totalReadLines += lineNumber;
		}
		progress.end();
		pw.close();
		System.out.println("Tatal read lines: " + totalReadLines);
		System.out.println("Tatal written lines: " + totalWrittenLines);
	}
