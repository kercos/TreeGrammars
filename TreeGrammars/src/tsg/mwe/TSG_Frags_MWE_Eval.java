...
public class TSG_Frags_MWE_Eval {
	
	static String mwePrefix = "MW";
	static int minMweCountToPrint = 10;

	private static void evalFrench() {
	...
	private static void evalFile(File evalFile, PrintWriter pw, String assMeas) throws FileNotFoundException, WrongParenthesesBlockException {
		...
		int thirdUnderscoreIndex = Utility.indexOf(file_name, '_', 4);
		int dotIndex = file_name.indexOf('.');
		String sign = file_name.substring(thirdUnderscoreIndex + 1, dotIndex);
		int signlength = Integer.parseInt(file_name.substring(thirdUnderscoreIndex-1, thirdUnderscoreIndex));
		...
		while(scan.hasNextLine()) {			
			...
			if (frag.label().startsWith(mwePrefix))
	...
	public static void main(String[] args) throws FileNotFoundException, WrongParenthesesBlockException {
		...
		//eval("/gardner0/data/TSG_MWE/FTB/frag_MWE_AM_2_10/");
		eval("/gardner0/data/TSG_MWE/FTB/frag_MWE_AM_2_10/");
	}
