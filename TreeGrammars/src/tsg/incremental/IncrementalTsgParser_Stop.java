...
import tsg.incremental.IncrementalTsgParser_Stop.ChartParserThread.ChartColumn.ChartState;
...
public class IncrementalTsgParser_Stop extends Thread {
	...
	public IncrementalTsgParser_Stop(FragmentExtractorFreq FE, File testTBfile, String outputPath) throws Exception {
		this.FE = FE;
	...
	private void initVariables() {
		...
		outputPath += "ITSG_STOP_wsj" + section + "_";
	...
	private void convertLexFragmentsToFringes() {
		...
		for (Entry<Label, int[]> e : FE.topFreq.entrySet()) {
			...
			FragFringe ff = FragFringe.getStopFringe(firstSub);
			TermLabel firstLexLabel = ff.yield[1];
			int freq = e.getValue()[0];
			TSNodeLabel stopFrag = FragFringe.getStopFragment(firstSub);
			Utility.putInMapDouble(firstSubFringeFragMap, ff, stopFrag, new double[] { freq });
			Utility.putInMapTriple(firstSubFringes, firstLexLabel, ff.firstTerminal(), ff, new double[] { freq, freq });
		}
		...
		Parameters.reportLineFlush("\t\tStop fringes (firs-sub category): " + Utility.tableIntToString(FE.topFreq));
	...
	private void convertTemplateFragmentsToFringes() {
		...
		for (Entry<FragFringe, HashMap<TSNodeLabel, double[]>> e : firstLexTemplateFragMap.entrySet()) {
			...
			double maxFreq = Utility.getMaxValue(e.getValue());
			double maxFreqMarg = Utility.getSumValue(e.getValue());
			Utility.putInMapTriple(firstLexFringesTemplates, firstPosLabel, ff.root(), ff, new double[] { maxFreq, maxFreqMarg });
		}
		...
		for (Entry<FragFringe, HashMap<TSNodeLabel, double[]>> e : firstSubTemplateFragMap.entrySet()) {
			...
			double maxFreq = Utility.getMaxValue(e.getValue());
			double maxFreqMarg = Utility.getSumValue(e.getValue());
			Utility.putInMapTriple(firstSubFringesTemplates, firstPosLabel, ff.firstTerminal(), ff, new double[] { maxFreq, maxFreqMarg });
		}
		...
		for (Entry<FragFringe, HashMap<TSNodeLabel, double[]>> e : firstWordFringesTemplatesFragMap.entrySet()) {
			...
			double maxFreq = Utility.getMaxValue(e.getValue());
			double maxFreqMarg = Utility.getSumValue(e.getValue());
			Utility.putInMapTriple(firstWordFirstLexFringesTemplates, firstPosLabel, ff.root(), ff, new double[] { maxFreq, maxFreqMarg });
		}
	...
	public HashMap<TSNodeLabel, double[]> getOriginalFrags(FragFringe ff, boolean firstWord) {
		...
		if (result == null && templateSmoothing) { // template
			...
			HashMap<TSNodeLabel, double[]> anonymizedFrags = firstWord ? 
					firstWordFringesTemplatesFragMap.get(ffCopy) : 
						(isFirstLex ? 
								firstLexTemplateFragMap.get(ffCopy) : 
								firstSubTemplateFragMap.get(ffCopy));
	...
	private void printFringesToFile() {
		...
		if (printTemplateFragsToFileWithProb && templateSmoothing) {
			if (firstLexFringesTemplates != null) {
				...
				printProbTable(firstLexFringesTemplates, firstLexTemplateFragMap, gericFragsForSmoothingFile, true);
			}
			...
			if (firstSubFringesTemplates != null) {
				...
				printProbTable(firstSubFringesTemplates, firstSubTemplateFragMap, gericFragsForSmoothingFile, true);
			}
			...
			if (firstWordFirstLexFringesTemplates != null) {
				...
				printProbTable(firstWordFirstLexFringesTemplates, firstWordFringesTemplatesFragMap, gericFragsForSmoothingFile, false);
			}
		}
	...
	protected class ChartParserThread extends Thread {
		...
		private void parse() {
			...
			if (parsingSuccess) {
				...
				if (computeMinRiskTree) {
					...
					sentenceLogProb = ProbBox.getTotalSumInnerOuterMargProb(chart.get(sentenceLength).scanStatesSet);
		...
		private void computePrefixProbabilities() {
			...
			for (int i=0; i < sentenceLength+1; i++) {
				...
				prefixProb[i] = ProbBox.getTotalSumForwardMargProbDouble(col.scanStatesSet);
			}
		}
		...
		*/
		...
		public void computeOuterProbabilities() {
			// outerMargProb
			ChartColumn lastColumn = chart.get(sentenceLength);
			HashMap<TermLabel, HashMap<ChartState, ProbBox>> endStates = lastColumn.scanStatesSet;
		...
		private void computeMinRiskParses() {
			...
			HashMap<FragFringe, HashMap<Integer, ArrayList<ChartState>>> fragFringChartStatesTable = 
				collectAliveStatesInFringeTableStartIndex();

			int i = 0;

			for (Entry<FragFringe, HashMap<Integer, ArrayList<ChartState>>> e : fragFringChartStatesTable.entrySet()) {
				FragFringe ff = e.getKey();
				boolean stopFringe = ff.isStopFragFringe();
				...
				for (Entry<Integer, ArrayList<ChartState>> f : fragFringeStatesStartIndex.entrySet()) {
					...
					HashMap<TSNodeLabel, double[]> fragAmbiguityTable = firstWord ? 
							fragTableProbFractionFirstWord : fragTableProbFractionFollowingWords;
					...
					HashMap<Integer, HashMap<Integer, ArrayList<SpanProb>>> termIndexStartSpanProbTable = 
						new HashMap<Integer, HashMap<Integer, ArrayList<SpanProb>>>(); //indexed on termIndex, startSpan
					...
					//add stop terminal node
					if (stopFringe) {
						SpanProb tspStop = new SpanProb(new int[] { sentenceLength, sentenceLength+1}, 0d);
						HashMap<Integer, ArrayList<SpanProb>> startSpanTable = new HashMap<Integer, ArrayList<SpanProb>>();
						ArrayList<SpanProb> arraySpan = new ArrayList<SpanProb>();
						arraySpan.add(tspStop);
						startSpanTable.put(sentenceLength, arraySpan);
						termIndexStartSpanProbTable.put(1, startSpanTable); //term at index 1
					}
					...
					for (Entry<TSNodeLabel, double[]> g : fragAmbiguityTable.entrySet()) {
						...
						for (Entry<Integer, HashMap<Integer, ArrayList<SpanProb>>> termIndexSpanArray : 
							termIndexStartSpanProbTable.entrySet()) {
		...
		class ChartColumn {
			...
			private void populateFirstLexFragFringeSet() {
				...
				if (nextWordIndex > 0) {
					...
					firstLexFragFringeSet = Utility.deepHashMapDoubleCloneArray(firstWordFirstLexFringes.get(nextWord));
				}
			...
			class ChartState {
				...
				public void partitionStartIndexElementInYieldBeforeDot(boolean partialDerivation, double fragMargProb, 
						HashMap<Integer, HashMap<Integer, ArrayList<SpanProb>>> termIndexStartSpanProbTable) {
	...
	public static void main(String[] args) throws Exception {
		...
		new IncrementalTsgParser_Stop(FE, testTB, workingPath).run();
	}
		...
		for (int ll : new int[] { 10 }) { // ,20,40,-1
			...
			for (int t : new int[] { 50 }) {
				...
				for (String[] set : settings) {
					...
					new IncrementalTsgParser_Stop(FE, testTB, outputPathParsing).run();
					System.gc();
				}
				// }
				// }
