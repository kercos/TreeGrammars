...
import tsg.incremental.IncrementalTsgEstimator.ProbBox;
import tsg.incremental.IncrementalTsgEstimator.ChartParserThread.ChartColumn;
import tsg.incremental.IncrementalTsgParserFromFile.ChartParserThread.ChartColumn.ChartState;
...
public class IncrementalTsgParserFromFile extends Thread {
	...
	int parsedTrees, interruptedTrees, indexLongestWordTime,
	...
	HashMap<FragFringe, HashMap<TSNodeLabel, double[]>> initFringeFragMap,
			firstWordFringesTemplatesFragMap, firstLexFringeFragMap,
			firstLexTemplateFragMap, firstSubFringeFragMap,
	...
	HashMap<TermLabel, HashMap<TermLabel, HashMap<FragFringe, double[]>>> initFringes,
	...
	String baseFragmentsPath;

	public IncrementalTsgParserFromFile(File initFragsFile, File firstLexFragsFile, 
			File firstSubFragsFile, File testTBfile, String outputPath,
			String baseFragmentsPath) throws Exception {
		this.initFragsFile = initFragsFile;
		this.firstLexFragsFile = firstLexFragsFile;
		this.firstSubFragsFile = firstSubFragsFile;
		this.testTBfile = testTBfile;
		this.testTB = Wsj.getTreebank(testTBfile);
		this.outputPath = outputPath;
		this.baseFragmentsPath = baseFragmentsPath;
	}
	...
	private void initVariables() {
		...
		outputPath += "ITSG_fromFile_wsj" + section + "_";
	...
	private void readFragFiles() {

		initFringeFragMap = new HashMap<FragFringe, HashMap<TSNodeLabel, double[]>>();
		...
		int count = readFragmentFile(initFragsFile, true, initFringeFragMap, false);
		Parameters.reportLineFlush("\tRead frags: " + count);

		Parameters.reportLineFlush("Reading first lex frags from " + firstLexFragsFile);
		count = readFragmentFile(firstLexFragsFile, true, firstLexFringeFragMap, false);
		Parameters.reportLineFlush("\tRead frags: " + count);

		Parameters.reportLineFlush("Reading first sub frags from " + firstSubFragsFile);
		count = readFragmentFile(firstSubFragsFile, false, firstSubFringeFragMap, false);
		Parameters.reportLineFlush("\tRead frags: " + count);
	...
	private static int readFragmentFile(File inputFile, boolean firstLex,  
			HashMap<FragFringe, HashMap<TSNodeLabel, double[]>> fragMapTable,
			boolean baseFrag) {
		...
		while (scan.hasNextLine()) {			
			...
			if (baseFrag) {
				if (!Utility.containsSecondKey(fragMapTable, ff, frag)) {
						continue;
				}
				//freq = baseFragsFreq;
			}
			//else {
				freq = Double.parseDouble(lineSplit[1]);
			//}			
			Utility.putInMapDouble(fragMapTable, ff, frag, new double[] { freq });
			count++;
		}
	...
	private void readBaseFragments() {
		if (baseFragmentsPath==null)
			return;
		
		Parameters.reportLineFlush("Reading base fringes... ");
		
		File baseInitFragsFile = new File(baseFragmentsPath + "_initFrags.txt");
		File baseFirstLexFragsFile = new File(baseFragmentsPath + "_firstLexFrags.txt");
		File baseFirstSubFragsFile = new File(baseFragmentsPath + "_firstSubFrags.txt");
				
		Parameters.reportLineFlush("Reading base init frags from " + baseInitFragsFile);
		int count = readFragmentFile(baseInitFragsFile, true, initFringeFragMap, true);
		Parameters.reportLineFlush("\tRead frags: " + count);
	
		Parameters.reportLineFlush("Reading base first lex frags from " + baseFirstLexFragsFile);
		count = readFragmentFile(baseFirstLexFragsFile, true, firstLexFringeFragMap, true);
		Parameters.reportLineFlush("\tRead frags: " + count);
	
		Parameters.reportLineFlush("Reading base first sub frags from " + baseFirstSubFragsFile);
		count = readFragmentFile(baseFirstSubFragsFile, false, firstSubFringeFragMap, true);
		Parameters.reportLineFlush("\tRead frags: " + count);
	
	}

	private void buildFringeTables() {
		
		Parameters.reportLineFlush("Building fringe tables...");
		
		initFringes = new HashMap<TermLabel, HashMap<TermLabel, HashMap<FragFringe, double[]>>>();
		...
		convertFragsToFringes(initFringes, initFringeFragMap, true);
		convertFragsToFringes(firstLexFringes, firstLexFringeFragMap, true);
		convertFragsToFringes(firstSubFringes, firstSubFringeFragMap, false);
		
	}

	private void reportFragFringesCounts() {
		int countInitFrags = Utility.countTotalDouble(initFringeFragMap);
		int countFirstLexFrags = Utility.countTotalDouble(firstLexFringeFragMap);
		int countFirstSubFrags = Utility.countTotalDouble(firstSubFringeFragMap);		
		int totalLexFrags = countInitFrags + countFirstLexFrags + countFirstSubFrags;
		int countInitFringes = initFringeFragMap.size();
		int countFirstLexFringes = firstLexFringeFragMap.size();
		int countFirstSubFringes = firstSubFringeFragMap.size();
		int totalLexFringes = countInitFringes + countFirstLexFringes + countFirstSubFringes;
	
		Parameters.reportLineFlush("Total number of lex fragments: " + totalLexFrags);
		Parameters.reportLineFlush("\tInit     : " + countInitFrags);
		...
		Parameters.reportLineFlush("\tInit     : " + countInitFringes);
	...
	private void convertFragsToFringes(
			HashMap<TermLabel, HashMap<TermLabel, HashMap<FragFringe, double[]>>> fringesTable,
			HashMap<FragFringe, HashMap<TSNodeLabel, double[]>> fragMapTable,			
			boolean firstLex) {
		
		// PUTTING HIGHEST AND MARGINAL FREQ PER FRINGE
		for (Entry<FragFringe, HashMap<TSNodeLabel, double[]>> e : fragMapTable.entrySet()) {
			FragFringe ff = e.getKey();
			TermLabel firstLexLabel = firstLex ? ff.firstTerminal() : ff.secondTerminal();
			double maxFreq = Utility.getMaxValue(e.getValue());
			double totFreq = Utility.getSumValue(e.getValue());
			TermLabel secondKey = firstLex ? ff.root() : ff.firstTerminal();
			Utility.putInMapTriple(fringesTable, firstLexLabel, secondKey, ff, new double[] { maxFreq, totFreq });
		}
	...
	private void convertFreqToLogRelativeFreq() {
		...
		// FIRST WORD
		double totalSumFirstWord = getTotalSummedProb(initFringeFragMap);
		convertToLogRelativeFreq(initFringes, totalSumFirstWord);
		convertToLogRelativeFreqFragMap(initFringeFragMap, totalSumFirstWord);
	...
	public HashMap<TSNodeLabel, double[]> getOriginalFrags(FragFringe ff, boolean firstWord) {
		...
		if (firstWord)
			result = initFringeFragMap.get(ff);
		...
		if (result == null) { // template
			...
			HashMap<TSNodeLabel, double[]> anonymizedFrags = firstWord ? firstWordFringesTemplatesFragMap.get(ffCopy) : (isFirstLex ? firstLexTemplateFragMap.get(ffCopy) : firstSubTemplateFragMap.get(ffCopy));
	...
	public double getOriginalMargProb(FragFringe ff, boolean firstWord) {
		...
		if (firstWord)
			result = initFringes.get(lexLabel).get(root).get(ff);
		else if (isFirstLex) {
	...
	private void printFringesToFile() {
		...
		if (printAllLexFragmentsToFileWithProb) {
			...
			printProbTable(initFringes, initFringeFragMap, outputFile, false);
		}
	...
	public static TSNodeLabel combineDerivation(ArrayList<TSNodeLabel> derivation) {
		...
		while (derivationIter.hasNext()) {
			...
			if (addSTOP && FragFringe.isStopFrag(nextFrag))
				continue;
	...
	protected class ChartParserThread extends Thread {
		...
		private void initVariables(ArrayList<Label> sentenceWords) {
			...
			wordTermLabels = new TermLabel[sentenceLength + 1];
			...
			wordTermLabels[i++] = FragFringe.STOPnodeLex;
			wordTermLabelsSet.add(FragFringe.STOPnodeLex);
			...
			fringeCounterFirstLexSubBin = new int[2][sentenceLength + 1];
		...
		private void parse() {
			...
			if (parsingSuccess) {
				...
				if (computeMinRiskTree) {
					...
					sentenceLogProb = ProbBox.getTotalSumInnerOuterMargProb(getEndStatesTable());
		...
		private void initChart() {
			...
			chart.add(new ChartColumn(FragFringe.STOPnodeLex, i));
		...
		private void computeMinRiskParses() {
			...
			MinRiskParseBuilder maxRiskParseBuilder = new MinRiskParseBuilder(sentenceLength+1, sentenceLogProb);
			...
			for (Entry<FragFringe, HashMap<Integer, ArrayList<ChartState>>> e : fragFringChartStatesTable.entrySet()) {
				...
				if (firstTermIsLex) {
					...
					fragTableProbFractionFirstWord = Utility.hashMapDoubleClone(getOriginalFrags(ff, true));
					// double margProbFirstWordAlt =
					// Utility.logSumHashMap(fragTableProbFractionFirstWord,0);
					// assert(Math.abs(margProbFirstWord-margProbFirstWordAlt)<0.0000001);
				...
				HashMap<TSNodeLabel, double[]> fragTableProbFractionFollowingWords = Utility.hashMapDoubleClone(getOriginalFrags(ff, false));
				// double margProbFollowingWordsAlt =
				// Utility.logSumHashMap(fragTableProbFractionFollowingWords,0);
				// assert(Math.abs(margProbFollowingWords-margProbFollowingWordsAlt)<0.0000001);
		...
		public HashMap<TermLabel, HashMap<ChartState, ProbBox>> getEndStatesTable() {
			ChartColumn lastColumn = chart.get(sentenceLength);
			return addSTOP ? lastColumn.scanStatesSet : lastColumn.subUpFirstStatesSet;
		}

		public ArrayList<ChartState> retrieveRandomViterbiPath(double[] viterbiLogProb) {
			HashMap<TermLabel, HashMap<ChartState, ProbBox>> endStates = getEndStatesTable();
			Entry<ChartState, ProbBox> topStateEntry = ProbBox.getMaxViterbiEntryDouble(endStates);
		...
		private void printDebug() {
			...
			if (limitTestToSentenceIndex != -1) {
				...
				this.toStringTexChartVertical(pw, derPaths, false, true, false, true);
				pw.close();
		...
		class ChartColumn {
			...
			private void populateFirstLexFragFringeSet() {
				...
				if (nextWordIndex > 0) {
					firstLexFragFringeSet = Utility.deepHashMapDoubleCloneArray(firstLexFringes.get(nextWord));
				} else {
					firstLexFragFringeSet = Utility.deepHashMapDoubleCloneArray(initFringes.get(nextWord));
				}
			...
			public void completeSubUpAndTerminate() {
				if (completeSlow)
					performCompleteSlow();
				else
					performCompleteFast();
			...
			public void terminate() {
				finished = true;
				parsingSuccess = addSTOP ? 
						!scanStatesSet.isEmpty() : !subUpFirstStatesSet.isEmpty();
			}
	...
	public static void main(String[] args) throws Exception {
		...
		String baseFragsPath = null; //fragEstimatorWorkingPath + "cycle_000"; 
		
		
		/*
		File initFragFile = new File(workingPath + "fragments_approxFreq" + "_initFrags.txt");
		File firstLexFragFile = new File(workingPath + "fragments_approxFreq" + "_firstLexFrags.txt");
		File firstSubFragFile = new File(workingPath + "fragments_approxFreq" + "_firstSubFrags.txt");
		workingPath += "Smoothing/SmoothingBest/";
		*/
		
		/*
		File initFragFile = new File(workingPath + "training_double_initFrags.txt");
		File firstLexFragFile = new File(workingPath + "training_double_firstLexFrags.txt");
		File firstSubFragFile = new File(workingPath + "training_double_firstSubFrags.txt");
		workingPath += "Smoothing/NoSmoothingSimulation/";
		*/
		
		
		workingPath += "Smoothing/ITSG_SmoothingExp_posConstraint_freq1000/";
		File initFragFile = new File(workingPath + "smoothing_initFrags.txt");
		File firstLexFragFile = new File(workingPath + "smoothing_firstLexFrags.txt");
		File firstSubFragFile = new File(workingPath + "smoothing_firstSubFrags.txt");
				
		
		String outputPathParsing = workingPath + "ParsingFromFile/";
		new File(outputPathParsing).mkdirs();
		
		treeLenghtLimit = 20;
		
		IncrementalTsgParserFromFile ITSP = 
			new IncrementalTsgParserFromFile(initFragFile, firstLexFragFile, firstSubFragFile, 
					testTB, outputPathParsing, baseFragsPath);
		
		ITSP.run();
		
		/*
		String fragEstimatorWorkingPath = workingPath + "Estimator/ITSG_Estimator/";
		File globalLog = new File(fragEstimatorWorkingPath + "log_cycles.txt");
		PrintWriter pw = FileUtil.getPrintWriter(globalLog);
		for(int i=0; i<=5; i++) {
			//int i=1;
			String cycle = Utility.padZero(3, i);
			File initFragFile = new File(fragEstimatorWorkingPath + "cycle_" + cycle + "_initFrags.txt");
			File firstLexFragFile = new File(fragEstimatorWorkingPath + "cycle_" + cycle + "_firstLexFrags.txt");
			File firstSubFragFile = new File(fragEstimatorWorkingPath + "cycle_" + cycle + "_firstSubFrags.txt");

			String outputPathParsing = fragEstimatorWorkingPath + "Parsing_" + cycle + "/";
			new File(outputPathParsing).mkdirs();
			...
			IncrementalTsgParserFromFile ITSP = 
				new IncrementalTsgParserFromFile(initFragFile, firstLexFragFile, firstSubFragFile, 
						testTB, outputPathParsing, baseFragsPath);
			ITSP.run();
			pw.append("Cycle " + cycle + "\n");
			pw.append("\tInit Frags:" + Utility.countTotalDouble(ITSP.initFringeFragMap) + "\n");
			pw.append("\tFirst Lex Frags:" + Utility.countTotalDouble(ITSP.firstLexFringeFragMap) + "\n");
			pw.append("\tFirst SUb Frags:" + Utility.countTotalDouble(ITSP.firstSubFringeFragMap) + "\n");			
			pw.append("\tParsed Trees:" + ITSP.parsedTrees + "\n");
			pw.append("\tMPD F1:" + Arrays.toString(ITSP.evalC_MPD.getRecallPrecisionFscoreAll()) + "\n");
			pw.append("\n");
			System.gc();
		}
		*/
