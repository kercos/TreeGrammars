...
import tsg.incremental.IncrementalTsgEstimator.ChartParserThread.ChartColumn.ChartState;
...
public class IncrementalTsgEstimator extends Thread {

	static boolean addSTOP = false;
	
	static int threads = 1;
	static int maxNumberCycle = 10;
	static long threadCheckingTime = 4000;

	static boolean extractOnlyMPD = true;
	...
	HashMap<Label, HashMap<Label, HashMap<TSNodeLabel, double[]>>> 
	...
	HashMap<Label, HashMap<Label, HashMap<TSNodeLabel, double[]>>> initFragmentsTmp,
			firstLexFragmentsTmp, firstSubFragmentsTmp;

	public IncrementalTsgEstimator(FragmentExtractorFreq FE, File trainingTBfile, String outputPath) throws Exception {
		this.FE = FE;
	...
	private void printParameters() {
		...
		Parameters.reportLine("Max Number of cycles: " + maxNumberCycle);
	...
	private void estimateFrequencies() {
		...
		int cycle = 0;
		printFragFrequencies(cycle);

		for (cycle = 1; cycle <= maxNumberCycle; cycle++) {
			convertFreqToLogRelativeFreq();
			extractNewFrequencies();
			initFragments = initFragmentsTmp;
			firstLexFragments = firstLexFragmentsTmp;
			firstSubFragments = firstSubFragmentsTmp;
			printFragFrequencies(cycle);
	...
	private void printFragFrequencies(int cycle) {
		Parameters.reportLineFlush("Printing fragments to files. ");
		Parameters.reportLineFlush("Cycle " + cycle);
		String cyclePad = Utility.padZero(3, cycle);
		File initFragFile = new File(outputPath + "cycle_" + cyclePad + "_initFrags.txt");
		File firstLexFragFile = new File(outputPath + "cycle_" + cyclePad + "_firstLexFrags.txt");
		File firstSubFragFile = new File(outputPath + "cycle_" + cyclePad + "_firstSubFrags.txt");
		int countInit = printToFile(initFragments, initFragFile);
		int countFirstLex = printToFile(firstLexFragments, firstLexFragFile);
		int countFirstSub = printToFile(firstSubFragments, firstSubFragFile);
	...
	private void extractNewFrequencies() {
		progress = new PrintProgress("Estimating from tree:", 1, 0);

		initFragmentsTmp = new HashMap<Label, HashMap<Label, HashMap<TSNodeLabel, double[]>>>();
		firstLexFragmentsTmp = new HashMap<Label, HashMap<Label, HashMap<TSNodeLabel, double[]>>>();
		firstSubFragmentsTmp = new HashMap<Label, HashMap<Label, HashMap<TSNodeLabel, double[]>>>();
	...
	protected class ChartParserThread extends Thread {
		...
		HashMap<Label, HashMap<Label, HashMap<TSNodeLabel, double[]>>> initFragmentsNewCounts,
				firstLexFragmentsNewCounts, firstSubFragmentsNewCounts;
		...
		private void initVariables() {
			...
			initFragmentsNewCounts = new HashMap<Label, HashMap<Label, HashMap<TSNodeLabel, double[]>>>();
			firstLexFragmentsNewCounts = new HashMap<Label, HashMap<Label, HashMap<TSNodeLabel, double[]>>>();
			firstSubFragmentsNewCounts = new HashMap<Label, HashMap<Label, HashMap<TSNodeLabel, double[]>>>();
		...
		private void parse() {
			...
			extractFragFrequencies();

			updateMainResults();

		}

		private void extractFragFrequencies() {
			// double treeProb = getTreeProbability();
			if (extractOnlyMPD) {
				double[] mpdProb = new double[1];
				ArrayList<ChartState> derivationPath = retrieveRandomViterbiPath(mpdProb);
				for (ChartState cs : derivationPath) {
					if (cs.isFirstStateWithCurrentWord())
						addFragFinalCount(cs, new double[] { 1 });
				}
			} else {
				computeOuterProbabilities();
				treeLogProb = ProbBox.getTotalSumInnerOuterMargProb(getEndStatesTable());
				// checkFinalProbConsistency();
				for (ChartColumn cc : chart) {
					Vector<HashMap<TermLabel, HashMap<ChartState, ProbBox>>> columnChartState = cc.getAllChartStateSets();
					for (HashMap<TermLabel, HashMap<ChartState, ProbBox>> csTable : columnChartState) {
						for (HashMap<ChartState, ProbBox> csTableValue : csTable.values()) {
							for (ChartState cs : csTableValue.keySet()) {
								if (cs.isFirstStateWithCurrentWord()) {
									ProbBox pb = cs.probBox;
									if (!pb.outerProb.isNaN()) {
										double freq = Math.exp(pb.inXout() - treeLogProb);
										addFragFinalCount(cs, new double[] { freq });
									}
								}
							}
						}
		...
		private void addFragFinalCount(ChartState cs, double[] prob) {
		...
		private void updateMainResults() {
			synchronized (progress) {
				Utility.increaseAllHashMapTriple(initFragmentsNewCounts, initFragmentsTmp);
				Utility.increaseAllHashMapTriple(firstLexFragmentsNewCounts, firstLexFragmentsTmp);
				Utility.increaseAllHashMapTriple(firstSubFragmentsNewCounts, firstSubFragmentsTmp);
				progress.next();
			}
		...
		public ArrayList<ChartState> retrieveRandomViterbiPath(double[] viterbiLogProb) {
			HashMap<TermLabel, HashMap<ChartState, ProbBox>> endStates = 
				getEndStatesTable();				
			Entry<ChartState, ProbBox> topStateEntry = 
				ProbBox.getMaxInnerViterbiEntryDouble(endStates);
	...
	public static void main(String[] args) throws Exception {
		...
		String outputPathParsing = workingPath + "Estimator/";
		new File(fragExtrWorkingPath).mkdirs();
		new File(outputPathParsing).mkdirs();

		FragmentExtractorFreq FE = FragmentExtractorFreq.getDefaultFragmentExtractor(fragExtrWorkingPath, "NoSmoothing", "NoSmoothing", trainTB, testTB, fragFile);

		FE.posSmoothing = false;
		new IncrementalTsgEstimator(FE, trainTB, outputPathParsing).run();		
