...
public class DepTreeCompareLabeled extends JPanel 
    ...
    private DepTreePanelLabeled goldTreePanel, testTreePanel;    			
    private JSplitPane splitPane;
    private JScrollPane goldScroller, testScroller;
    private JCheckBox skewedLinesCheckBox;
    private JTextField uasScore, lasScore;
    private JLabel sizeReport;
    private int currentSentenceNumber;
    private int size;
    //private File goldFile, testFile;
    private ArrayList<DSConnl> goldTreebank, testTreebank;
    
    private static DSConnl initialSentence = new DSConnl(
    		new String[]{"Load", "gold_and_test", "corpora"},
    		new String[]{"V","J","N"},    		
    		new int[]{0,3,1},
    		new String[]{"ROOT", "NMOD", "OBJ"}
    );    

    public DepTreeCompareLabeled() {
        super(new BorderLayout());        
        size = 1;
        goldTreebank = new ArrayList<DSConnl>();
        testTreebank = new ArrayList<DSConnl>();
    ...
    private void loadGoldCorpus(File goldFile) {
    	goldTreebank.clear();
    	goldTreebank = DSConnl.getConnlTreeBank(goldFile);		
    }
    
    private void loadtestCorpus(File testFile) {
    	testTreebank.clear();
    	testTreebank = DSConnl.getConnlTreeBank(testFile);	
    }
    ...
    private void buildComponents() {
        ...
        skewedLinesCheckBox = new JCheckBox("Skewed Lines:"); 
        ...
        JLabel lasLabel = new JLabel("LAS: ");
        lasScore = new JTextField(7);
        lasLabel.setFocusable(false);
        lasScore.setFocusable(false);
        lasScore.setEditable(false);
        
        rightControlPanel.add(uasLabel);
        rightControlPanel.add(uasScore);
        rightControlPanel.add(lasLabel);
        rightControlPanel.add(lasScore);
        rightControlPanel.add(controlSentenceNumber);
        sizeReport = new JLabel();        
        rightControlPanel.add(sizeReport);              
				
        //Set up the drawing area.
        goldTreePanel = new DepTreePanelLabeled();   
        ...
        testTreePanel = new DepTreePanelLabeled();
    ...
    private void init() {    	
    	DSConnl s1 = goldTreePanel.getSentence(currentSentenceNumber);
    	DSConnl s2 = testTreePanel.getSentence(currentSentenceNumber);
        BitSet[] wrongIndexesPosLabels = DSConnl.wrongIndexesPosLabels(s1, s2);
    	goldTreePanel.init();    	
    	testTreePanel.setWrongIndexesPosLabels(wrongIndexesPosLabels);
        testTreePanel.init();                
        int length = s1.length;
        int uas = length - wrongIndexesPosLabels[0].cardinality();
        BitSet wrongIndexesLabels = new BitSet();
        wrongIndexesLabels.or(wrongIndexesPosLabels[0]);
        wrongIndexesLabels.or(wrongIndexesPosLabels[2]);        
        int las = length - wrongIndexesLabels.cardinality();
        uasScore.setText(uas + "/" + length);
        lasScore.setText(las + "/" + length);
    }
    ...
    private static void createAndShowGUI() {
		...
		final DepTreeCompareLabeled contentPane = new DepTreeCompareLabeled();
