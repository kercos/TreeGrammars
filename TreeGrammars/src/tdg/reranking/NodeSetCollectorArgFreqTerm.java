 ...
 *
 */
public class NodeSetCollectorArgFreqTerm extends NodeSetCollector{
	
	ArrayList<Duet<BitSet, int[]>> bitSetTable;
	
	public NodeSetCollectorArgFreqTerm() {
		super();
	...
	public Object clone() {
		NodeSetCollectorArgFreqTerm clone = new NodeSetCollectorArgFreqTerm();
	...
	public boolean addAll(NodeSetCollector collector) {
		...
		NodeSetCollectorArgFreqTerm coll = (NodeSetCollectorArgFreqTerm)collector;
		...
		for(int[] comb : combinations) {
			...
			for(int i=0; i<collectors.length; i++) {
				...
				Duet<BitSet, int[]> choosenElement = 
					((NodeSetCollectorArgFreqTerm)collectors[i]).bitSetTable.get(comb_i);
	...
	public double getProbArgFrame(NodeSetCollector[] wordsKernels, TDNode[] structure,
			TDNode tree, Hashtable<String, Double> substitutionCondFreqTableDouble, int SLP_type) {
		...
		double[] globalSubProbArray = new double[wordsKernels.length];
		Arrays.fill(globalSubProbArray, -1);
		
		double gloablSubstProb =	getGlobalSubstProb(tree, globalSubProbArray,
					wordsKernels, structure, substitutionCondFreqTableDouble, SLP_type);
		
		return gloablSubstProb;
		
	}
	
	private static double getGlobalSubstProb(TDNode tree, double[] globalSubstProbArray,
			NodeSetCollector[] wordsKernels, TDNode[] structure, 
			Hashtable<String, Double> substitutionCondFreqTableDouble, int SLP_type) {
		
		int thisIndex = tree.index;
		double globalSubProb = globalSubstProbArray[thisIndex];
		if (globalSubProb != -1) return globalSubProb;
		
		NodeSetCollectorArgFreqTerm thisKernel = (NodeSetCollectorArgFreqTerm)wordsKernels[thisIndex];
		if (thisKernel.isEmpty()) {
			if (tree.hasArgumentDaughters()) return globalSubstProbArray[thisIndex] = 0d;
			globalSubProb = 1d;
		}
		else {		
			globalSubProb = 0;
			for(Duet<BitSet, int[]> element : thisKernel.bitSetTable) {
				BitSet bs = element.getFirst();
				double subFreq = element.getSecond()[0];
				TDNode[] kenelLeaves = getKernelLeaves(structure, bs);
				
				int internalNodes = bs.cardinality() - kenelLeaves.length - 1;
				double subFreqDiscount = Math.pow(2, -internalNodes) * subFreq;
				String condFreqKey = removeArgMark(tree.lexPosTag(SLP_type));
				double subCondFreq = substitutionCondFreqTableDouble.get(condFreqKey);
				double derivationProb = subFreqDiscount / subCondFreq;
														
				for(TDNode leafNode : kenelLeaves) {				
					double subProbNode = getGlobalSubstProb(leafNode, globalSubstProbArray,
							wordsKernels, structure, substitutionCondFreqTableDouble, SLP_type);
		...
		TDNode[] adjunctDaughters = tree.getAdjunctDaughters();
		for(TDNode adjDaughter : adjunctDaughters) {
			globalSubProb *= getGlobalSubstProb(adjDaughter, globalSubstProbArray,
					wordsKernels, structure, substitutionCondFreqTableDouble, SLP_type);
		}
	...
	public static TDNode[] getKernelLeaves(TDNode[] structure, BitSet kernel) {
		Vector<TDNode> leavesNodes = new Vector<TDNode>();
		...
		while( (index = kernel.nextSetBit(index)) != -1) {			
			if (index < length) {
				...
				if (isLeafOfKernel(t, kernel)) leavesNodes.add(t);
			}
			index++;
		}		
		return leavesNodes.toArray(new TDNode[leavesNodes.size()]);
	}
	
	public static boolean isLeafOfKernel(TDNode t, BitSet kernel) {
